...
class Storage(object):
    ...
    def __init__(self, flow_detail, connection, scope_fetcher=None):
        ...
        self._transients = {}
        self._injected_args = {}
        self._lock = lockutils.ReaderWriterLock()
        self._ensure_matchers = [
            ((task.Task,), (middleware.TaskDetail, 'Task')),
            ((retry.Retry,), (middleware.RetryDetail, 'Retry')),
        ]
        ...
        try:
            source, _clone = self._atomdetail_by_name(
                self.injector_name, expected_type=middleware.TaskDetail)
        except exceptions.NotFound:
            pass
        else:
    ...
    def _with_connection(self, functor, *args, **kwargs):
        ...
        return functor(self._connection, *args, **kwargs)
    ...
    def set_task_progress(self, task_name, progress, details=None):
        ...
        self._update_atom_metadata(task_name, update_with,
                                   expected_type=middleware.TaskDetail)
    ...
    def get_task_progress(self, task_name):
        ...
        """
        source, _clone = self._atomdetail_by_name(
            task_name, expected_type=middleware.TaskDetail)
        try:
    ...
    def save_retry_failure(self, retry_name, failed_atom_name, failure):
        ...
        source, clone = self._atomdetail_by_name(
            retry_name, expected_type=middleware.RetryDetail, clone=True)
        try:
    ...
    def get_retry_history(self, retry_name):
        ...
        source, _clone = self._atomdetail_by_name(
            retry_name, expected_type=middleware.RetryDetail)
    ...
    def get_retry_histories(self):
        ...
        histories = []
        for ad in self._flowdetail:
            if isinstance(ad, middleware.RetryDetail):
