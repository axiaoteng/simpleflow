...
from simpleflow.engines import base
from simpleflow.engines import runtime
from simpleflow.engines import builder
from simpleflow.engines import compiler
from simpleflow.engines import executor
...
class ActionEngine(base.Engine):
    ...
    **Engine options:**
    ...
    def __init__(self, flow, flow_detail, connection, options):
        super(ActionEngine, self).__init__(flow, flow_detail, connection, options)
        self._runtime = None
        self._compiled = False
        self._compilation = None
        self._compiler = compiler.PatternCompiler(flow)
        self._lock = lockutils.RLock()
    ...
    def run(self, timeout=None):
        ...
        """
        with lockutils.try_lock(self._lock) as was_locked:
            if not was_locked:
...
class SerialActionEngine(ActionEngine):
    ...
    def __init__(self, flow, flow_detail, session, options):
...
class ParallelActionEngine(ActionEngine):
    ...
    def __init__(self, flow, flow_detail, connection, options):
        max_workers = options.pop('max_workers', None) if options else None
        ...
        self._task_executor = executor.ParallelGreenThreadTaskExecutor(max_workers)
